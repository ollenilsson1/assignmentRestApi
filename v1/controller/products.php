...
if (array_key_exists("productid", $_GET)) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        try {
            ...
            $query = $DB->prepare('SELECT COUNT(product_id) AS totalProducts FROM products');
            $query->execute();

            $row = $query->fetch(PDO::FETCH_ASSOC);

            $productsCount = intval($row['totalProducts']);

            $numOfPages = ceil($productsCount / $limitPerPage); //ceil gör att numret alltid rundas uppåt, 1.5 blir 2 tex.
            ...
            $query = $DB->prepare('SELECT product_id, title, description, imgUrl, price, quantity, DATE_FORMAT(created_at, "%d/%m/%Y %H:%i") as created_at, DATE_FORMAT(updated_at, "%d/%m/%Y %H:%i") as updated_at FROM products limit :pglimit offset :offset');
